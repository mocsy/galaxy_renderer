...
void NBodyWnd::Render() {
  ...
  switch (m_camOrient) {
    ...
    case 1: {
      Vec2D p1 = m_galaxy.GetStarPos(0), p2 = m_galaxy.GetStarPos(1);
      orient.x = p2.x - p1.x;
      orient.y = p2.y - p1.y;
      orient.z = 0;
    }
    ...
    case 2: {
      Vec2D p1 = m_galaxy.GetStarPos(0), p2 = m_galaxy.GetStarPos(2);
      orient.x = p2.x - p1.x;
      orient.y = p2.y - p1.y;
      orient.z = 0;
    }
...
void NBodyWnd::DrawDensityWaves(int num, double rad) {
  ...
  for (int i = 0; i <= num; ++i) {
    double r = (i + 1) * dr;
    glColor3f(1, 1, 1);
    DrawEllipsis(r, r * m_galaxy.GetExcentricity(r), Constant::RAD_TO_DEG * m_galaxy.GetAngularOffset(r));
  }
}
...
void NBodyWnd::DrawStars() {
  ...
  for (int i = 1; i < num; ++i) {
    ...
    if (m_starRenderType == 1) {
      glColor3f(col.r * pStars [i].m_mag, col.g * pStars [i].m_mag, col.b * pStars [i].m_mag);
    }
...
void NBodyWnd::DrawDust() {
  ...
  for (int i = 0; i < num; ++i) {
    ...
    glColor3f(col.r * pDust [i].m_mag, col.g * pDust [i].m_mag, col.b * pDust [i].m_mag);
    glVertex3f(pos.x, pos.y, 0.0f);
...
void NBodyWnd::DrawH2() {
  ...
  for (int i = 0; i < num; ++i) {
    ...
    glColor3f(col.r * pH2 [i].m_mag * 2, col.g * pH2 [i].m_mag, col.b * pH2 [i].m_mag);
