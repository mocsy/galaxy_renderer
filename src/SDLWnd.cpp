...
void SDLWindow::SaveToTGA(const std::string &sName) {
  ...
  char header [6] = { GetWidth() % 256, GetWidth() / 256, GetHeight() % 256, GetHeight() / 256, 24, 0 };
...
void SDLWindow::AdjustCamera() {
  ...
  double l = m_fov / 2.0;
  glOrtho(-l, l, -l, l, -l, l);
  gluLookAt(m_camPos.x, m_camPos.y, m_camPos.z, m_camLookAt.x, m_camLookAt.y, m_camLookAt.z, m_camOrient.x, m_camOrient.y, m_camOrient.z);
